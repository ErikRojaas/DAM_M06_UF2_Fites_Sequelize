...
// Importar models
const { Youtuber, PerfilYoutuber, Video, Categoria, VideosCategories, Usuari, Comentari, Valoracio } = require('./src/models');
...
/**
 * Carrega els usuaris
 * @param {Array} usuaris Dades dels usuaris
 */
 ...
 * @param {Array} comentaris Dades dels comentaris
 */
  ...
  try {
    ...
    for (const comentari of comentaris) {
      await Comentari.create({
        ...
        text: comentari.text,
        data_comentari: comentari.data_comentari
      });
    }
...
/**
 * Carrega les valoracions
 * @param {Array} valoracions Dades de les valoracions
 */
async function carregarValoracions(valoracions) {
  try {
    ...
    for (const valoracio of valoracions) {
      // Verificar que el campo 'tipus' no esté vacío o nulo
      if (!valoracio.tipus || !['like', 'dislike'].includes(valoracio.tipus)) {
        logger.warn(`Valoració no vàlida: ${JSON.stringify(valoracio)}`);
        continue; // Ignorar esta fila si el valor es inválido
      }
...
async function carregarTotesDades() {
  try {
    ...
    logger.info("Totes les dades han estat carregades correctament!");
